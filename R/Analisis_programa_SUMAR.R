...
programa <- pdftools::pdf_text("programas/programa_SUMAR.pdf")
programa
...
frases_texto<-data.frame(frases_texto[3:nrow(frases_texto),])
#   2. Quitamos el epílogo y todo lo que viene al final
frases_texto<-data.frame(frases_texto[1:4428,])
...
ggplot(frecuencia_palabras[1:10,], aes(x = reorder(word, -n), y = n)) +
  ...
  labs(title = "Palabras más comunes en el programa de SUMAR",
       x = "Palabra",
       y = "Frecuencia") +
...
paresPalabras %>%
  ...
  geom_node_text(aes(label = name), repel = TRUE, 
                 point.padding = unit(0.2, "lines")) +
  ggtitle('Bigramas programa electoral SUMAR')
...
top_terms %>%
  ...
  labs(x = NULL, y = "Importancia palabras en topic",
       title = paste0("Topics y sus palabras descriptivas SUMAR"))
...
ggplot(frecuencia_framing[1:num_categorias_mostrar,], aes(x = reorder(word, -n), y = n)) +
  geom_col(fill = "steelblue") +
  labs(title = "Framing en el Programa Político SUMAR",
       x = "Categoría",
       y = "Frecuencia") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

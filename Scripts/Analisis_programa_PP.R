...
programa <- pdftools::pdf_text("programas/programapp.pdf")
programa
...
frases_texto<-data.frame(frases_texto[10:nrow(frases_texto),])
#   2. Quitamos el epílogo y todo lo que viene al final
frases_texto<-data.frame(frases_texto[1:1019,])
...
ggplot(frecuencia_palabras[1:10,], aes(x = reorder(word, -n), y = n)) +
  ...
  labs(title = "Palabras más comunes en el programa de PP",
       x = "Palabra",
       y = "Frecuencia") +
...
print(paresPalabras, n=100)

# Eliminar bigramas que contengan únicamente dígitos
paresPalabras <- palabras %>% 
  pairwise_count(word, ID, sort = TRUE, upper = FALSE)

# Eliminar bigramas que contengan únicamente dígitos
paresPalabras <- paresPalabras %>%
  filter(!str_detect(item1, "^\\d+$") & !str_detect(item2, "^\\d+$"))
...
paresPalabras %>%
  ...
  geom_node_text(aes(label = name), repel = TRUE, 
                 point.padding = unit(0.2, "lines")) +
  ggtitle('Bigramas programa electoral PP')
...
top_terms %>%
  ...
  labs(x = NULL, y = "Importancia palabras en topic",
       title = paste0("Topics y sus palabras descriptivas PP"))
...
categorias <- c("tecnología", "medioambiente", "vivienda", "educación", "justicia", "igualdad", "inmigrantes", "salud", "seguridad", "impuestos")
...
ggplot(frecuencia_framing[1:num_categorias_mostrar,], aes(x = reorder(word, -n), y = n)) +
  geom_col(fill = "steelblue") +
  labs(title = "Framing en el Programa Político PP",
       x = "Categoría",
       y = "Frecuencia") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

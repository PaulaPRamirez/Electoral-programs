...
programa <- pdftools::pdf_text("programas/programa_PSOE.pdf")
programa
...
frases_texto<-data.frame(frases_texto[6:nrow(frases_texto),])
#   2. Quitamos el epílogo y todo lo que viene al final
frases_texto<-data.frame(frases_texto[1:2755,])
...
palabras <- palabras %>%
  filter(!(word %in% c("2023", "julio", "bloque")))
...
ggplot(frecuencia_palabras[1:10,], aes(x = reorder(word, -n), y = n)) +
  ...
  labs(title = "Palabras más comunes en el programa de PSOE",
       x = "Palabra",
       y = "Frecuencia") +
...
top_terms %>%
  ...
  labs(x = NULL, y = "Importancia palabras en topic",
       title = paste0("Topics y sus palabras descriptivas PSOE"))



#Analisis framing 
#Identificar y analizar cómo se presenta la información en un texto para influir en la pfraercepción del lector o audiencia
...
categorias <- c("tecnología", "ecológica", "vivienda", "educación", "justicia", "mujeres", "migrantes", "salud", "seguridad", "impuestos")
...
ggplot(frecuencia_framing[1:num_categorias_mostrar,], aes(x = reorder(word, -n), y = n)) +
  geom_col(fill = "steelblue") +
  labs(title = "Framing en el Programa Político PSOE",
       x = "Categoría",
       y = "Frecuencia") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
